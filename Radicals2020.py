...
df = pd.read_csv("data_radicals2020.csv", index_col=0)
...
LogReg = LogisticRegression(penalty='l1', solver='liblinear', verbose=False) # sparsity
...
for C in C_grid:
    LogReg = LogisticRegression(penalty='l1', C=C, solver='liblinear', verbose=False) 
...
best_c_index = np.where(C_grid == best_params['C']) # C = 0.74989
...
plt.ylim([-2, 2.05])
plt.text(47.5, 1, "Radical", fontsize=12, rotation=-90)
plt.text(47.5, -1.3, "Non radical", fontsize=12, rotation=-90)
plt.tight_layout()
